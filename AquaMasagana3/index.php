...
<head>
    ...
    <style>
        *{box-sizing:border-box;font-family:Arial,Helvetica,sans-serif}
        body{margin:0;background:#f2f9ff;color:#333}
        header{background:#0a6cff;color:#fff;padding:20px 40px;display:flex;justify-content:space-between;align-items:center;position:fixed;top:0;left:0;width:100%;z-index:1000}
        .logo{display:flex;align-items:center;gap:10px}
        .logo-circle{width:40px;height:40px;border-radius:50%;background:linear-gradient(to bottom,#4fc3f7,#0a6cff)}
        nav a{color:#fff;margin-left:20px;text-decoration:none;font-weight:bold}
        nav a:hover{text-decoration:underline}
        .hero{padding:90px 40px;text-align:center;background:linear-gradient(to right,#0a6cff,#3fa9f5);color:#fff}
        .hero h1{font-size:42px;margin-bottom:10px}
        .hero p{font-size:18px}
        section{padding:60px 40px;max-width:1100px;margin:auto}
        h2{color:#0a6cff;margin-bottom:20px}
        .services{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:20px}
        .card{background:#fff;padding:20px;border-radius:12px;box-shadow:0 4px 12px rgba(0,0,0,.1)}
        .card h3{margin-top:0}
        .order-form,.contact-form{background:#fff;padding:25px;border-radius:12px;box-shadow:0 4px 12px rgba(0,0,0,.1)}
        input,textarea,select{width:100%;padding:10px;margin-bottom:15px;border-radius:6px;border:1px solid #ccc}
        button{background:#0a6cff;color:#fff;border:none;padding:12px 20px;border-radius:6px;font-size:16px;cursor:pointer}
        button:hover{background:#084ecb}
        footer{background:#0a6cff;color:#fff;text-align:center;padding:20px;margin-top:40px}
    </style>
</head>
<body>
